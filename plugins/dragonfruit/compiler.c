...
char *read_file(char *path) {
	...
	size_t length;
	{
		int l = ftell(f);
		if (l == -1) {
			compiler_error("Error obtaining size of file %s!", path);
			fclose(f);
			return NULL;
		}
		length = l;
	...
	if (length > MAX_DF) {
		compiler_error("Source file %s is too big!", path);
		fclose(f);
		return NULL;
	}
	rewind(f);
	...
	size_t read;
	if ((read = fread(buf, 1, length, f)) != length) {
		compiler_error("Error reading from source file %s! Expected length: %" PRIu64 ", read: %" PRIu64 ", buffer: '%s'", path, length, read, buf);
		fclose(f);
		free(buf);
		return NULL;
	}
