...
static void function_call(const char *name) {
	for (uint8_t i = 6; i < cur_auto; i++) {
		if (cur_auto > MAX_AUTO) {
			compiler_error("INTERNAL ERROR: more autos assigned than the maximum allowed!");
		}
		else {
			char buf[16];
