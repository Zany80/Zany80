...
int convert(list_t *sources, const char *target, char **buffer) {
	...
	if (setjmp(env) == 0) {
		...
		zany_log(ZL_ERROR, "scas aborted\n");
		ret = -1;
	}
...
void scas_logv(int verbosity, const char *format, va_list args) {
	...
	if (l < 0) {
		zany_log(ZL_ERROR, "Error appending to scas log\n");
	}
	else {
		if (l >= 1024) {
			zany_log(ZL_WARN, "scas log message too big; dropped\n");
		}
...
void set_verbosity(int verbosity) {
	if (verbosity > 1)
		verbosity--;
	zany_log(ZL_DEBUG, "[scas] verbosity is at %d\n", verbosity);
